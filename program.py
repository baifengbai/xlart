...
def image_to_xlsx(image_file, xlsx_file):
    ...
    for i in range(img.width):
        for j in range(img.height):
            ws.cell(row=j+1, column=i+1).fill = PatternFill(fgColor=Color(
                "{0:02x}{1:02x}{2:02x}".format(pixel[i, j][0], pixel[i, j][1], pixel[i, j][2])), fill_type='solid')
...
if __name__ == '__main__':
    start = time.time()
    resize_image('picture.jpg', 'picture_resized.jpg', 256, 384)
    image_to_xlsx('picture_resized.jpg', 'demo.xlsx')
    end = time.time()
